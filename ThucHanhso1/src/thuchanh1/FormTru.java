...
try {
    String textSo1 = txtSo1.getText();
    String textSo2 = txtSo2.getText();
    double so1 = Double.parseDouble(textSo1);
    double so2 = Double.parseDouble(textSo2);
    double ketQua = so1 - so2;
    lblKQ.setForeground(Color.BLACK);
    lblKQ.setText(so1 + " - " + so2 + " = " + ketQua);
} catch (NumberFormatException e) {
    lblKQ.setForeground(Color.RED);
    lblKQ.setText("Nhập dữ liệu sai. Vui lòng nhập số.");
} catch (Exception e) {
    lblKQ.setForeground(Color.RED);
    lblKQ.setText("Đã xảy ra lỗi.");
}
